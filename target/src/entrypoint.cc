...
uint32_t receive_rotary_encoder_buffer[2];
...
void motor_control_loop() {
    ...
    set_motor_speed(speed_output_3508 * 1, speed_output_6020);
}
...
void handle_command_data() {
    ...
    move_speed_target = (receive_rotary_encoder_buffer[0]) / 4095.;
    receive_rotary_encoder_buffer[1] >= 2048 ? rotate_speed_target = (2048 - receive_rotary_encoder_buffer[1]) / 2048. : rotate_current_speed = -(receive_rotary_encoder_buffer[1] - 2048) / 2048.;
}
...
void entrypoint() {
    ...
    HAL_ADC_Start_DMA(&hadc1, receive_rotary_encoder_buffer, sizeof(receive_rotary_encoder_buffer) / sizeof(receive_rotary_encoder_buffer[0]));
